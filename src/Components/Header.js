...
function Header() {
  ...
  const toggleDarkMode = () => {
    ...
    document.documentElement.classList.toggle("dark"); // Toggle dark mode class on html element
  };
  ...
  return (
    <header className="container mx-auto md:flex justify-between py-2 max-width">
      <div className="flex justify-between items-center py-2 ">
        <NavLink to="/">
          {darkMode ? (
            <img className="w-[128px]" src={logos.logoWhite} alt="logo" />
          ) : (
            <img className="w-[128px]" src={logos.logoDark} alt="logo" />
          )}
        </NavLink>
        <div className="flex gap-4 items-center">
          <div onClick={toggleDarkMode} className="cursor-pointer lg:hidden" >
            {darkMode ? <MdDarkMode size={24} color="white" /> : <MdOutlineDarkMode size={24} />}
          </div>
      ...
      <nav className={` ${!isOpen ? "hidden" : null} text-center md:flex justify-between`}>
        ...
        <ul className="flex justify-evenly items-center my-5 md:my-0 md:space-x-5 md:mr-5">
          ...
          <li>
            <div onClick={toggleDarkMode} className="cursor-pointer hidden lg:block" >
              {darkMode ? <MdDarkMode size={24} color="white" /> : <MdOutlineDarkMode size={24} />}
            </div>
          </li>
        </ul>
